...
def read_input_file(infile):
    ...
    nprops = int(lines[0].strip())
    materialprops = []
    for prop in lines[1].split():
        try:
            materialprops.append(float(prop))
        except ValueError:
            pass

    # read nodal coordinates
    ncoord = int(lines[nprops+1].strip())
    coords = []
    for line in lines[nprops+2:nprops+2+ncoord]:
        coords.append([float(x) for x in line.split()])

    # read element connectivity
    nelem = int(lines[nprops+2+ncoord].strip())
    connect = []
    nelnodes = []
    for line in lines[nprops+3+ncoord:nprops+3+ncoord+nelem]:
        cellarray = line.split()
        nelnodes.append(int(cellarray[0]))
        connect.append([int(cellarray[i]) for i in range(1, nelnodes[-1]+1)])

    # read boundary conditions
    nfix = int(lines[nprops+3+ncoord+nelem].strip())
    fixnodes = []
    for line in lines[nprops+4+ncoord+nelem:nprops+4+ncoord+nelem+nfix]:
        fixnodes.append(int(line))

    # read loads
    ndload = int(lines[nprops+4+ncoord+nelem+nfix].strip())
    dloads = []
    for line in lines[nprops+5+ncoord+nelem+nfix:nprops+5+ncoord+nelem+nfix+ndload]:
        dloads.append([float(x) for x in line.split()])

    ndof = 2  # number of degrees of freedom per node
    nnode = len(coords)  # total number of nodes
    maxnodes = max(nelnodes)  # maximum number of nodes per element
    elident = list(range(1, nelem+1))  # element identifiers
